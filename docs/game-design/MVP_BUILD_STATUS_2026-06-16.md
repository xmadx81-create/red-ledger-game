...
🗺 월드맵(섬 개방)
   → ⚔ 스테이지 전투(섬멸전/설득전, 승리)
      → 🎁 카드 보상 적립 + 스테이지 클리어(섬 개방)
         → ⚗ 합성소(N 모아 상위 등급 도약, 천장)
            → 🧪 배치 육성소(스탯 키워 자유 직업 부여 → 파티)
               → 다시 전투 …  (종료 없음)
```

상태(보유 카드·파티·진행도)는 `localStorage`로 화면 간 공유된다.
...
| 🧪 배치 육성소 | `job-lab.html` | 장소 배치→스탯 재분배→직업 후보→파티 편성 |
| ⚔ 전투 | `battle-prototype.html` | 턴제 카드 전투(섬멸/설득), 스킬·시너지·오토, 보상·클리어 |
...
| `battle-prototype.json` | 전투 셋업(파티/적/행동/스킬/시너지/설득전) |
...
```text
hbh.inventory.v1  보유 카드(등급별 카운트)+합성 천장/통계  — 합성소·육성소·전투(보상) 공유
...
- 전투/설득전 밸런스 v0(섬멸 ~70%, 설득은 직업 구성 의존).
```
...
```text
1. 카드 인스턴스 식별 도입 → 히든 합성 트리 실제 구현(현재 fungible 카운트라 미지원)
2. 전설 5장/히든 체인의 '카드 확정'(현재 placeholder)
3. 잠입전 등 추가 멀티 장르 모드
4. 합성으로 만든 상위 카드를 도감/스튜디오에 전시
5. v0 수치 밸런스(합성 확률·budget·일일 N 공급·시즌 길이 연동)
6. 대륙/섬 실제 콘텐츠·맵 이동 연출
7. 온라인/소셜(길드·랭킹·PvP) 범위 결정
8. (마지막) 캐릭터/카드/맵 이미지 — Batch 단위·톤 가이드 준수
```
...
# 브라우저에서 src/web-mvp/hub.html 진입 → 월드맵/합성소/육성소/전투로 이동
```
